...
impl VirtqDesc {
    ...
    /// descriptor.
    fn new(cpu: &mut impl Cpu, addr: u32) -> Result<Self, Exception> {
        Ok(Self {
            addr: cpu.read(addr, Sizes::Word, AccessType::Writable)?,
            len: cpu.read(addr.wrapping_add(8), Sizes::Word, AccessType::Writable)?,
            flags: cpu.read(addr.wrapping_add(12), Sizes::HalfWord, AccessType::Writable)?,
            next: cpu.read(addr.wrapping_add(14), Sizes::HalfWord, AccessType::Writable)?,
        })
    }
}
...
impl VirtqAvail {
    fn new(cpu: &mut impl Cpu, addr: u32) -> Result<Self, Exception> {
        Ok(Self {
            flags: cpu.read(addr, Sizes::HalfWord, AccessType::Writable)? as u16,
            idx: cpu.read(addr.wrapping_add(2), Sizes::HalfWord, AccessType::Writable)? as u16,
...
impl Virtio {
    ...
    pub fn initialize(&mut self, binary: &[u8]) {
    ...
    pub fn disk_access(cpu: &mut Riscv32Cpu) -> Result<(), Exception> {
        // https://docs.oasis-open.org/virtio/virtio/v1.1/csprd01/virtio-v1.1-csprd01.html#x1-1460002
        ...
        let avail = VirtqAvail::new(cpu.get_interface(), virtq.avail_addr)?;
        let _ = avail.flags;

        let head_index = cpu.read(
        ...
        // First descriptor.
        let desc0 = VirtqDesc::new(
            cpu.get_interface(),
            virtq.desc_addr + VRING_DESC_SIZE * head_index,
        )?;
        assert_eq!(desc0.flags & VIRTQ_DESC_F_NEXT, 1);

        // Second descriptor.
        let desc1 = VirtqDesc::new(
            cpu.get_interface(),
            virtq.desc_addr + VRING_DESC_SIZE * desc0.next,
        )?;
        ...
        match (desc1.flags & VIRTQ_DESC_F_WRITE) == 0 {
            true => {
                ...
                for i in 0..desc1.len {
                    let data = cpu.read(desc1.addr + i, Sizes::Byte, AccessType::Writable)?;
            ...
            false => {
                ...
                for i in 0..desc1.len {
                    ...
                    cpu.write(desc1.addr + i, data, Sizes::Byte, AccessType::Writable)?;
                }
            }
        };

        // Third descriptor address.
        let desc2 = VirtqDesc::new(
            cpu.get_interface(),
            virtq.desc_addr + VRING_DESC_SIZE * desc1.next,
        )?;
        assert_eq!(desc2.flags & VIRTQ_DESC_F_NEXT, 0);
        // Tell success.
        cpu.write(desc2.addr, 0, Sizes::Byte, AccessType::Writable)?;
